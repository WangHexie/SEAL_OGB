...
def create_dataset(number_of_nodes):
    ...
    #self connected graph edges
    edges+= np.random.randint(number_of_a+number_of_b, number_of_a+number_of_b+number_of_self_connected, size=(int(len(edges)/2), 2)).tolist()
    
    full_node_number = number_of_a+number_of_b+number_of_self_connected
    adj = np.zeros((full_node_number, full_node_number))
    ...
    number_of_test_edges = int(number_of_nodes**1.5)
    # create positive and negative test edge in bipartite graph
    pos_edges = []
    neg_edges = np.random.choice(range(number_of_a), (number_of_test_edges, 2))
    ...
    # create positive and negative test edge in self-connected graph
    
    node_1 = np.random.randint(number_of_a+number_of_b, number_of_a+number_of_b+number_of_self_connected,number_of_test_edges).tolist()
    node_2 = np.random.randint(number_of_a+number_of_b, size=number_of_test_edges).tolist()
    neg_edges = np.concatenate((neg_edges, np.array([node_1, node_2]).T), axis=0)
    
    
    pos_edges_temp = []
    for i in range(number_of_a+number_of_b, number_of_a+number_of_b+number_of_self_connected):
      for j in range(number_of_a+number_of_b, number_of_a+number_of_b+number_of_self_connected):
        if len(pos_edges_temp) == number_of_test_edges:
            break
        if adj[i,j] == 0:
          pos_edges_temp.append([i, j])
    
#     print(pos_edges.shape())
    pos_edges = np.concatenate((pos_edges, np.array(pos_edges_temp)), axis=0)
    ...
    data = Data(x=torch.tensor([[0]]*(full_node_number), dtype=torch.float), num_nodes=full_node_number, split_edge=split_edge, edge_index=torch.tensor(edges, dtype=torch.long).t())
    return data
